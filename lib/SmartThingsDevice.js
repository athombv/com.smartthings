...
// Note: debug devices at https://my.smartthings.com/advanced/devices.
...
module.exports = class SmartThingsDevice extends OAuth2Device {
  ...
  async onOAuth2Init() {
    ...
    await this.oAuth2Client.registerDevice({
      ...
      this.error(`Error Registering Device: ${err.message}`);
      this.setWarning(`Device updates may be delayed.\nSmartThings Server Error: ${err.message}`).catch(() => { });
  ...
  async onOAuth2Uninit() {
    ...
    await this.oAuth2Client.unregisterDevice({
      deviceId,
    }).catch(err => this.error(`Error Unregistering Device: ${err.message}`));
  }
  ...
  onStatus(status) {
    if (process.env.DEBUG === '1') {
      this.log('onStatus', JSON.stringify(status));
    }
