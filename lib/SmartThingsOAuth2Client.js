...
module.exports = class SmartThingsOAuth2Client extends OAuth2Client {
  ...
  async createSubscription({
    ...
    if (!token.installed_app_id) {
      throw new Error('Missing installed_app_id in token'); // TODO
    }

    await this.post({
      path: `/v1/installedapps/${token.installed_app_id}/subscriptions`,
      json: {
        sourceType: 'DEVICE',
        device: {
          deviceId,
  ...
  async registerDevice({
    ...
    const token = await this.getToken();
    if (!token.installed_app_id) {
      throw new Error('Missing installed_app_id in token');
    }
